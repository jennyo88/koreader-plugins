...
local function details_for(book)
    ...
    if author and author ~= "" then table.insert(bits, author) end
    if book.pages then table.insert(bits, tostring(book.pages) .. " pages") end

    if book.series and book.series ~= "" then
        local s = book.series
        if book.series_index then s = s .. " #" .. tostring(book.series_index) end
        table.insert(bits, s)
    end
...
function TBRRecommender:showRecommendations(header, books)
    ...
    for i, book in ipairs(books) do
        table.insert(lines, string.format("%d. %s", i, title_for(book)))
        local details = details_for(book)
        if details ~= "" then table.insert(lines, "   " .. details) end
        if i < #books then table.insert(lines, "") end
    end
...
function TBRRecommender:runMode(mode)
    local books = Library:getCandidates()

    if #books == 0 then
        UIManager:show(InfoMessage:new{
            text = _("No unread EPUBs were found."),
        })
        return
    end

    if mode == "surprise" then
        self:showRecommendations(_("SURPRISE ME"), Recommender:surpriseMe(books, 3))
    elseif mode == "quick" then
        self:showRecommendations(_("QUICK READ"), Recommender:quickRead(books, 3))
    elseif mode == "series" then
        self:showRecommendations(_("CONTINUE A SERIES"), Recommender:continueSeries(books, 3))
    elseif mode == "unopened" then
        self:showRecommendations(_("UNOPENED BOOKS"), Recommender:unopened(books, 3))
    end
end
...
function TBRRecommender:addToMainMenu(menu_items)
    menu_items.tbr_recommender = {
        ...
        sorting_hint = "tools",
        sub_item_table = {
            { text = _("Surprise Me"), callback = function() self:runMode("surprise") end },
            { text = _("Quick Read"), callback = function() self:runMode("quick") end },
            { text = _("Continue a Series"), callback = function() self:runMode("series") end },
            { text = _("Unopened Books"), callback = function() self:runMode("unopened") end },
            {
                text = _("About"),
                callback = function()
                    UIManager:show(InfoMessage:new{
