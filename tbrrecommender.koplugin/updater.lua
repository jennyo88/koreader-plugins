...
Updater.plugin_id = "tbrrecommender"
Updater.plugin_name = "TBR Recommender"
...
Updater.plugin_url =
    Updater.base_url .. "/tbrrecommender.koplugin"
...
local function download(url)
    ...
    local _, code, _, status = https.request{
        ...
        headers = {
            ["User-Agent"] = "KOReader-TBRRecommender-Updater",
            ["Accept"] = "*/*",
        },
    }
...
function Updater:getManifest()
    local body, err = download(self.manifest_url)
    ...
    if not plugin then
        return nil,
            "TBR Recommender was not found in manifest."
    end
...
function Updater:createBackup(files)
    ...
    local ok, err =
        ensure_clean_directory(backup_dir)
...
function Updater:confirmRestore()
    UIManager:show(
        ConfirmBox:new{
            text =
                _(
                    "Restore the previous version of TBR Recommender?"
                ),
            ok_text =
                _("Restore"),
            cancel_text =
...
function Updater:performUpdate(plugin)
    local progress =
        InfoMessage:new{
            text =
                _("Downloading TBR Recommender update…"),
        }

    UIManager:show(progress)

    local downloaded, download_err =
        self:downloadFiles(plugin.files)
    ...
    local installed, install_err =
        self:installFiles(plugin.files)

    if not installed then
        UIManager:show(
            InfoMessage:new{
                text =
                    _("Update could not be installed.\n\n")
    ...
    UIManager:show(
        ConfirmBox:new{
            text =
                string.format(
                    _(
                        "TBR Recommender %s installed successfully.\n\nThis will take effect on next restart."
                    ),
                    tostring(plugin.version)
                ),
            ok_text =
                _("Restart now"),
            cancel_text =
    ...
    UIManager:show(
        ConfirmBox:new{
            text =
                string.format(
                    _(
                        "A TBR Recommender update is available.\n\nInstalled: %s\nAvailable: %s"
                    ),
                    tostring(installed_version),
                    tostring(plugin.version)
                ),
