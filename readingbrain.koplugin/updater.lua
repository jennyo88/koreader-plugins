...
Updater.plugin_id = "readingbrain"
Updater.plugin_name = "Reading Brain"
...
Updater.plugin_url =
    Updater.base_url .. "/readingbrain.koplugin"
...
local function download(url)
    ...
    local _, code, _, status = https.request{
        ...
        headers = {
            ["User-Agent"] = "KOReader-ReadingBrain-Updater",
            ["Accept"] = "*/*",
        },
    }
...
function Updater:getManifest()
    ...
    if not plugin then
        return nil,
            "Reading Brain was not found in manifest."
    end
...
function Updater:confirmRestore()
    UIManager:show(
        ConfirmBox:new{
            text =
                _(
                    "Restore the previous version of Reading Brain?"
                ),
            ok_text =
                _("Restore"),
            cancel_text =
...
function Updater:performUpdate(plugin)
    local progress =
        InfoMessage:new{
            text =
                _("Downloading Reading Brain update…"),
        }
    ...
    UIManager:show(
        ConfirmBox:new{
            text =
                string.format(
                    _(
                        "Reading Brain %s installed successfully.\n\nThis will take effect on next restart."
                    ),
    ...
    UIManager:show(
        ConfirmBox:new{
            text =
                string.format(
                    _(
                        "A Reading Brain update is available.\n\nInstalled: %s\nAvailable: %s"
                    ),
                    tostring(installed_version),
                    tostring(plugin.version)
                ),
